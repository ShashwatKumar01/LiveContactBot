...
def main_menu_kb() -> InlineKeyboardMarkup:
    ...
    b.row(styled_btn("➕ Add Bot", "add_bot"), styled_btn("🤖 My Bots", "my_bots"))
...
def bots_list_kb(bots: list[dict]) -> InlineKeyboardMarkup:
    ...
    b.row(styled_btn("➕ Add Bot", "add_bot"))
...
def bot_settings_kb(bot_doc: dict) -> InlineKeyboardMarkup:
    ...
    b = InlineKeyboardBuilder()
    b.row(styled_btn("📝 Texts", f"bot_texts:{bot_id}"), styled_btn("👥 Groups", f"groups:{bot_id}"))
    b.row(styled_btn("📢 Broadcast", f"bc_start:{bot_id}"), styled_btn("📊 Statistics", f"stats:{bot_id}"))
    b.row(
    ...
    b.row(styled_btn("🔌 Disconnect Bot", f"disconnect:{bot_id}"))
...
def bot_texts_kb(bot_id: int) -> InlineKeyboardMarkup:
    b = InlineKeyboardBuilder()
    b.row(styled_btn("Welcome (/start)", f"text_edit:{bot_id}:start"))
...
def auto_cooldown_kb(bot_id: int) -> InlineKeyboardMarkup:
    ...
    b.row(
        ...
        styled_btn("1 hour", f"auto_cd_set:{bot_id}:3600"),
        styled_btn("2 hours", f"auto_cd_set:{bot_id}:7200"),
    )
    b.row(styled_btn("6 hours", f"auto_cd_set:{bot_id}:21600"), styled_btn("24 hours", f"auto_cd_set:{bot_id}:86400"))
    b.row(styled_btn("« Back", f"bot:{bot_id}"))
    return b.as_markup()
...
def confirm_disconnect_kb(bot_id: int) -> InlineKeyboardMarkup:
    b = InlineKeyboardBuilder()
    b.row(
        styled_btn("✅ Yes, disconnect", f"confirm_disconnect:{bot_id}"),
